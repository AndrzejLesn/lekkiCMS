<?php   
    ...
    if ($_GET['choose'] == NULL)
    {
      ...
      $num = NULL;
		  ...
		  $result .= '<input type="text" name="subject" value="'.$sub.'" />';
		  //Content
		  $result .= '<label>'.$lang['contact']['content'].'</label>';
		  $result .= '<textarea name="content">'.$num.'</textarea>';
		  //Hidden input
		  $result .= '<input type="text" name="captcha" style="display:none;" />';
    }
    else
    {
      $sub = 'Zamówienie pakietu '.$_GET['choose'];
      $num = ' ,Nr zamówienia : '.md5(time());
      //Subject
		  $result .= '<label>Informacje</label>';
		  $result .= '<input type="text" name="subject" value="'.$sub.$num.'" readonly/>';
		  //Hidden input
		  $result .= '<input type="text" name="captcha" style="display:none;" />';
